...
test_assert_same_file(){
	...
	h_rm gggggg/uuu/ii
	assert_exit_code "assert_same_file gggggg/uuu/ii aa" 99 > /dev/null 2>&1
	h_cp ${SSU_HOME}/ssu.jar hoge.jar
	assert_same_file ${SSU_HOME}/ssu.jar hoge.jar
...
test_assert_not_same_file(){
	...
	h_cp ${SSU_HOME}/ssu.jar hoge.jar
	assert_exit_code "assert_not_same_file ${SSU_HOME}/ssu.jar hoge.jar" 99 > /dev/null 2>&1
...
test_assert_include_in_file(){
	...
	assert_exit_code "assert_include_in_file poohpoohpooh ${SSU_HOME}/ssu.jar" 99 > /dev/null 2>&1
	...
	assert_exit_code "assert_include_in_file poohpoohpooh ${SSU_HOME}/ssu.jar pooh" 99 > aa 2> /dev/null
	my_assert_log aa
	assert_include_in_file pooh aa
	
	assert_exit_code "assert_include_in_file poohpoohpooh ${SSU_HOME}/ssu.jar pooh 1" 99 > aa 2> /dev/null
	assert_include_in_file "Wrong Arguments" aa
...
date
#SSU_TEST_PATTERN="test_assert_blank_file"
#SSU_DEBUG_MODE=ON
startSSU;
date
